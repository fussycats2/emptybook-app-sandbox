...
| **DB 스키마** | `0001_init.sql` 전체 + `0002_books_cover_url.sql` (cover_url 컬럼). RLS·Realtime·Storage 포함 |
...
| **마이페이지 화면** | `/mypage/selling` (내 책 + 상태 칩 필터), `/mypage/likes` (찜한 책 그리드). STATS 4종 중 판매중·구매내역·찜은 `listMyBooks`/`listOrders`/`listLikedBookIds` 실데이터, "최근 본"은 placeholder. 미구현 SECTIONS 항목(받은 후기/최근 본 상품/쿠폰함/공지·문의·약관)은 "준비중" 칩 + 토스트 |
...
| **알림 푸시 / 모두 읽음 서버 반영** | 없음 | 알림 페이지 "모두 읽음"은 클라이언트 상태만 갱신. Edge Functions 미작성 |
| **프로필 수정** | UI만 | `/mypage/settings` 저장 로직 없음. 알림/개인정보 토글도 클라이언트만 |
| **받은 후기 화면** | 진입점만 | 마이페이지 "받은 후기" 항목은 "준비중" 칩 + 토스트로 노출 (`listReceivedReviews` 미구현) |
...
| **`profiles.rating_avg` 자동 갱신** | 없음 | 후기 저장 후 평균 별점 동기화 트리거 미작성 |
...
4. **받은 후기 화면** — `listReceivedReviews(userId)` + `/mypage/reviews` 화면 → 마이페이지 "받은 후기" 메뉴 연결
5. **`profiles.rating_avg` 자동 갱신** — 후기 INSERT 시 트리거 또는 Edge Function
6. **알림 푸시 / 모두 읽음** — Supabase Edge Functions + `notifications.read_at` 일괄 갱신
7. **결제 PG 연동** — 현재 Mock UI를 토스/카카오페이 등으로 교체
