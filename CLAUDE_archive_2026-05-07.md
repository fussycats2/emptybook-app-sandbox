...
> 최종 업데이트: 2026-05-07 (v9.9 — 설정에서 관심 장르 편집)
...
| **설정에서 관심 장르 편집 (v9.9)** | `profiles.preferred_genres` 가 회원가입(`/signup` step 1) 에서만 설정 가능하고 가입 후엔 수정처가 없던 문제. `/mypage/settings` 의 "프로필 정보" 그룹 아래에 "관심 장르" 그룹 추가 — `meta.CATEGORIES` 의 8개 카테고리 칩(이모지+이름) 다중 선택 + "관심 장르 저장" 버튼. **Repo**: `updateMyProfile` 시그니처에 `preferred_genres?: string[] \| null` 추가, payload 에 동적으로 포함. mock 폴백은 null → [] 로 정규화 (Profile 타입은 string[] 만 허용). **훅**: `useUpdateMyProfile.onSuccess` 가 `queryKeys.profile.me()` 외에 `queryKeys.book.lists()` 도 invalidate — 장르 변경 시 홈 카테고리 추천 섹션이 다음 진입에 새 장르로 자동 fetch. **Dirty 처리**: 장르는 텍스트 필드(displayName/username/phone) 와 분리된 별도 저장 버튼 — 정렬 무관 집합 비교(`some !includes` 양방향) 로 변경 감지. 빈 배열 저장도 허용 (사용자가 "추천 안 받기" 의도) |
| **홈 피드 큐레이션 (v9.8)** | 단순 최신순이던 홈 피드를 발견 경험 중심으로 보강. **(1) 지역 우선 정렬**: `listRecentBooks(limit, region?)` 가 region 인자를 받아 같은 동네 책을 먼저 N개, 부족하면 다른 지역으로 채움. region 매물이 limit 보다 많으면 그 안에서만 노출 → "{region}의 따끈한 책" 라벨이 정확. mock 모드도 동일 로직(`region` 또는 `loc` 매칭). React Query 캐시 키에 region 포함 (`queryKeys.book.recent(limit, region)`) → 사용자가 동네 칩으로 바꾸면 새 fetch. **(2) 카테고리 추천 미니 섹션**: 메인 피드 아래 + 인기 판매자 위에 가로 스크롤 카드 8장. 카테고리는 `profile.preferred_genres[0]` 가 있으면 그것(라벨 "취향에 맞을 X 책"), 없으면 기본 "소설"(라벨 "X 책 둘러보기"). 신규 사용자는 가입 시 선택한 장르가 자연스럽게 첫 추천이 됨 — 0001 의 `preferred_genres` 컬럼이 활용처를 얻음. 우측 "더 보기 →" 칩으로 `/search?category=...` 직진. **MiniBookCard**: 130px 폭 표지 강조 카드 — 메인 피드의 BookFeedItem 보다 넓이 줄여 발견 경험에 집중. 표지 위에 sold/reserved/free 상태 배지(좌상단). 호버 시 살짝 위로 떠오름. **searchBooks 활용**: 카테고리 필터는 기존 `useSearchBooks({ category })` 그대로 사용 — 신규 repo 함수 추가 없이 .slice(0, 8) 만 |
...
| 함수 | 설명 |
|------|------|
| `listRecentBooks(limit?, region?)` | 홈 피드. region 이 있으면 그 지역 책을 먼저 N개, 부족하면 다른 지역으로 채워 (v9.8) |
...
| `getMyProfile()` / `updateMyProfile(input)` / `updateAppPrefs(prefs)` | 내 프로필 조회/수정 + app_prefs 토글. updateMyProfile 의 input 은 `display_name`/`username`/`phone`/`preferred_genres` (v9.9) — preferred_genres 변경 시 useUpdateMyProfile.onSuccess 가 book lists 캐시도 invalidate |
